    ...
    if((do_change || do_remove) && totp_cli_ensure_authenticated(plugin_state, cli)) {
        ...
        do {
            ...
            if (backup_path != NULL) {
                ...
                TOTP_CLI_PRINTF_WARNING("Once you make sure everything is fine and works as expected, please delete this backup file\r\n");
                free(backup_path);
            } else {
                memset_s(&new_pin[0], TOTP_IV_SIZE, 0, TOTP_IV_SIZE);
                TOTP_CLI_PRINTF_ERROR("An error has occurred during taking backup of config file\r\n");
                break;
            }
