...
void totp_cli_command_move_docopt_commands() {
    TOTP_CLI_PRINTF("  " TOTP_CLI_COMMAND_MOVE ", " TOTP_CLI_COMMAND_MOVE_ALT "         Move\\rename token\r\n");
}
...
void totp_cli_command_move_docopt_options() {
    ...
    TOTP_CLI_PRINTF("  " DOCOPT_OPTION(
        ...
        DOCOPT_ARGUMENT(
            TOTP_CLI_COMMAND_MOVE_ARG_NEW_INDEX)) "     New token index.\r\n");
}
    ...
    while(args_read_string_and_trim(args, temp_str)) {
        ...
        if(furi_string_cmpi_str(temp_str, TOTP_CLI_COMMAND_MOVE_ARG_NEW_NAME_PREFIX) == 0) {
            if(!args_read_string_and_trim(args, temp_str)) {
                TOTP_CLI_PRINTF("Missed value for argument \"" TOTP_CLI_COMMAND_MOVE_ARG_NEW_NAME_PREFIX
                                "\"\r\n");
            } else {
                ...
                strlcpy(new_token_name, furi_string_get_cstr(temp_str), furi_string_size(temp_str) + 1);
                parsed = true;
            }
            ...
            if(!args_read_int_and_trim(args, &new_token_index)) {
                TOTP_CLI_PRINTF("Missed value for argument \"" TOTP_CLI_COMMAND_MOVE_ARG_NEW_INDEX_PREFIX
                                "\"\r\n");
            } else if (new_token_index < 1 || new_token_index > plugin_state->tokens_count) {
                TOTP_CLI_PRINTF(
                    "\"%" PRId16 "\" is incorrect value for argument \"" TOTP_CLI_COMMAND_MOVE_ARG_NEW_INDEX_PREFIX
                    "\"\r\n",
                    new_token_index);
            } else {
                parsed = true;
