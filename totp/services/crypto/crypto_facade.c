...
uint8_t* totp_crypto_encrypt(
    ...
    if (crypto_version == 2) {
        return totp_crypto_encrypt_v2(plain_data, plain_data_length, iv, key_slot, encrypted_data_length);
    }
...
uint8_t* totp_crypto_decrypt(
    ...
    if (crypto_version == 1) {
        return totp_crypto_decrypt_v1(encrypted_data, encrypted_data_length, iv, decrypted_data_length);
    }

    if (crypto_version == 2) {
        return totp_crypto_decrypt_v2(encrypted_data, encrypted_data_length, iv, key_slot, decrypted_data_length);
    }

    furi_crash("Unsupported crypto version");
}

CryptoSeedIVResult totp_crypto_seed_iv(PluginState* plugin_state, const uint8_t* pin, uint8_t pin_length) {
