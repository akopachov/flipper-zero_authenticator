...
void ui_control_vscroll_render(Canvas* const canvas, uint8_t x, uint8_t y, uint8_t height, uint8_t position, uint8_t max_position) {
    ...
    uint8_t block_position_y = height * ((float)position / (float)max_position) - (block_height >> 1);
    ...
    canvas_draw_box(canvas, x - (UI_CONTROL_VSCROLL_WIDTH >> 1), block_position_y_abs, UI_CONTROL_VSCROLL_WIDTH, block_height);
}
