...
static TotpIteratorUpdateTokenResult add_token_handler(TokenInfo* tokenInfo, const void* context) {
    ...
    if(context_t->scene_state->type == TokenTypeHOTP && sscanf(context_t->scene_state->initial_counter, "%" PRIu64, &tokenInfo->counter) != 1) {
    ...
    if(event->input.type == InputTypePress) {
        ...
        case InputKeyRight:
            if(scene_state->selected_control == TokenAlgoSelect) {
                ...
                totp_roll_value_uint8_t(
                    &scene_state->digits_count_index, 1, 0, COUNT_OF(TOKEN_DIGITS_TEXT_LIST) - 1, RollOverflowBehaviorRoll);
            } else if(
                scene_state->selected_control == TokenDurationOrCounterSelect &&
                scene_state->type == TokenTypeTOTP) {
                totp_roll_value_uint8_t(
                ...
                totp_roll_value_uint8_t(&scene_state->type, 1, 0, COUNT_OF(TOKEN_TYPE_LIST) - 1, RollOverflowBehaviorRoll);
            }
            break;
        case InputKeyLeft:
            if(scene_state->selected_control == TokenAlgoSelect) {
