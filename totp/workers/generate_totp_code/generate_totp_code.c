...
static void generate_totp_code(TotpGenerateCodeWorkerContext* context, const TokenInfo* token_info, uint32_t current_ts) {
    if(token_info->token != NULL && token_info->token_length > 0) {
        ...
        int_token_to_str(0, context->string, token_info->digits, token_info->algo);
    }
}
...
static int32_t totp_generate_worker_callback(void* context) {
    ...
    while(true) {
        ...
        TokenInfo* token_info = *(t_context->token_info);
        if (token_info == NULL) {
            continue;
        }
        ...
        if(flags & TotpGenerateCodeWorkerEventForceUpdate || (time_left = (curr_ts % token_info->duration) == 0)) {
            if(furi_mutex_acquire(t_context->update_sync, FuriWaitForever) == FuriStatusOk) {
                generate_totp_code(t_context, token_info, curr_ts);
                curr_ts = furi_hal_rtc_get_timestamp();
                furi_mutex_release(t_context->update_sync);
                (*(t_context->on_new_code_generated_handler))(time_left, t_context->on_new_code_generated_handler_context);
            }
        }

        (*(t_context->on_code_lifetime_changed_handler))(
            (float)(token_info->duration - curr_ts % token_info->duration) / (float)token_info->duration,
...
TotpGenerateCodeWorkerContext* totp_generate_code_worker_start(
    ...
    context->update_sync = update_sync;
